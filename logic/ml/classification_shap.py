...
class IDEARs_funcs(object):
	...
	def __init__(self):
		...
		self.path=self.path_data
		#self.path_figures= "/Users/michaelallwright/Documents/GitHub/UKB/PD/figures/"
		self.path_figures= "/Users/michaelallwright/Documents/analysis/ukb/ad/figures/"

		self.path_figures_dem= "/Users/michaelallwright/Documents/analysis/ukb/"
		self.path_figures_pd= "/Users/michaelallwright/Documents/analysis/ukb/"
		self.path_figures_pain="/Users/michaelallwright/Documents/analysis/ukb/" 
	...
	def borutafeats(self,df,remwords,depvar='AD'):
		rf = RandomForestClassifier(n_jobs=-1, class_weight='balanced', max_depth=5)
		feat_selector = BorutaPy(rf, n_estimators='auto', verbose=0, random_state=1)

		# find all relevant features - 5 features should be selected
		remcols=[c for c in df.columns if re.search(remwords,c)]
		df.drop(columns=remcols,inplace=True)
		feat_selector.fit(df.drop(columns=depvar).fillna(0).values, 
						  df[depvar].values)
		...
		borutacols=['eid',depvar]
		for i,col in enumerate([col for col in df.columns if col!=depvar]):
			if feat_selector.support_[i]==True:
				borutacols.append(col)
		return feat_selector.support_,borutacols,df_boruta
	...
	def simple_eval(self,df,model,dropcols,depvar,wordsremove,resizeratio,shapshow=1,preprocess=True,resize=0):
		...
		if shapshow==1:
			...
			expected_value = explainer.expected_value
			shap_values = explainer.shap_values(X)
			importances = pd.DataFrame(data={'Attribute': X.columns,'Importance': model.feature_importances_})
			importances = importances.sort_values(by='Importance', ascending=False)
			return shap_values, X, df_out,importances
	...
	def ABS_SHAP(self,df_shap,df,max_disp=20,figx=10,figy=10,dpi=200,plot=True,figname='shap_bar',format_file='.jpg',stream=False):
		...
		for i in feature_list:
			...
			corr_list.append(b)
		corr_df = pd.concat([pd.Series(feature_list),pd.Series(corr_list)],axis=1)
	...
	def run_entire_data_pd(self,df,drops,wordsremove,outfile,savefile=False,df_val_use=None,shapshow=1,
		...
		for i in range(runs):
			...
			if df_val_use is None and shapshow==2:

				shap_values, X, df_out,importances=self.simple_eval(df=df_val1,model=mod1,dropcols=drops,
				wordsremove=wordsremove,depvar=depvar,resize=resize,resizeratio=resizeratio,
			shapshow=shapshow,preprocess=preprocess)
			elif df_val_use is None:
				shap_values, X, df_out=self.simple_eval(df_val1,model=mod1,dropcols=drops,
				wordsremove=wordsremove,depvar=depvar,resize=resize,resizeratio=resizeratio,
			shapshow=shapshow,preprocess=preprocess)
		...
		if shapshow==2:
			shap_tuple=[shap_values_list,X_list,df_out_list,mod_list,imp_list]
	...
	def plot_ROCAUC_mult(self,y_test,y_score,labels,cols ,figname='check',figx=6,figy=4,format_out='.svg',stream=False,savefig=False):
		...
		if savefig:
			plt.savefig(self.path_figures+figname+self.date_run+format_out, dpi=300,bbox_inches='tight')
			plt.show()
