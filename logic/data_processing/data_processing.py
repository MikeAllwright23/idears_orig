...
class data_proc_main(object):
	...
	def __init__(self):
		...
		self.path="/Users/michaelallwright/Documents/data/ukb/full_data/"

		#temporarily going to need this one
		
		self.path2="/Users/michaelallwright/Documents/github/ukb/ukb-dementia-shap/static/"
	...
	def inf_to_null(self,df):

		"""
		helper function to convert inf vals values to nulls
		"""
		for c in df.columns:
			mask=(df[c]==np.inf)
			df[c][mask]=np.nan
	...
	def ukb_icd10(self):
		df=pd.read_csv(self.path_data+'metadata/icd10list_ukb.csv')
